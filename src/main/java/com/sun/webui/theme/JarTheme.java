...
public class JarTheme implements Theme {
    ...
    private static final Logger LOG = Logger.getLogger(JarTheme.class.getName());
    ...
    public String getPathToJSFile(String key) {
        LOG.finest(() -> String.format("getPathToJSFile(key=%s)", key));
    ...
    private String getPathToStylesheet(FacesContext context) {
        LOG.finest(() -> String.format("getPathToStylesheet(context=%s)", context));
        ClientType clientType = ClientSniffer.getClientType(context);
        try {
            String path = stylesheets.getString(clientType.toString());
            LOG.finest(() -> String.format("client type=%s, path=%s", clientType, path));
    ...
    private String getPathToStylesheet(String clientName) {
        LOG.finest(() -> String.format("getPathToStylesheet(clientName=%s)", clientName));
        try {
    ...
    public String getPathToTemplate(String clientName) {
        LOG.finest(() -> String.format("getPathToTemplate(clientName=%s)", clientName));
    ...
    protected void configureMessages(ResourceBundle base, ResourceBundle override) {
        LOG.finest(() -> String.format("configureMessages(base=%s, override=%s)", base, override));
        if (override == null) {
            bundle = base;
        } else {
            bundle = override;
    ...
    private String translateURI(String uri) {
        LOG.finest(() -> String.format("translateURI(uri=%s)", uri));
     ...
     * <p>
     * <ul>
     * <li>{@link #ALT_SUFFIX}</li>
     * <li>{@link ThemeImage#TITLE_SUFFIX}</li>
     * <li>{@link #HEIGHT_SUFFIX}</li>
     * <li>{@link #WIDTH_SUFFIX}</li>
     * <li>{@link ThemeImage#UNITS_SUFFIX}</li>
     * </ul>
