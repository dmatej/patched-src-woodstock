...
public class JarThemeFactory implements ThemeFactory {
    ...
    private static final Logger LOG = Logger.getLogger(JarThemeFactory.class.getName());
    ...
    private Iterator getThemeAttributes(ClassLoader classLoader) {
        LOG.finest(() -> String.format("getThemeAttributes(classLoader=%s)", classLoader));
    ...
    private static Set getLocales(Application application) {
        LOG.finest(() -> String.format("getLocales(application=%s)", application));
        ...
        if (locale != null) {
            log("Default locale is %s", locale);
        ...
        if (localeIterator != null) {
            while (localeIterator.hasNext()) {
                ...
                log("Adding locale %s", localeObject);
        ...
        while (themeVersion == null && manifests.hasMoreElements()) {
            ...
            try {
                ...
                if (attr != null) {
                    ...
                    log("Found attribute %s", themeVersion);
                }
            } catch (IOException e) {
                LOG.log(Level.SEVERE, "Could not detect theme version", e);
            } finally {
                try {
                    if (in != null) {
                        in.close();
    ...
    private Enumeration getManifests(ClassLoader classLoader) {
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "IOException using the Context ClassLoader", ioex);
        }
        if (manifests == null || !manifests.hasMoreElements()) {
            try {
                manifests = classLoader.getResources(MANIFEST);
            } catch (IOException ioex) {
                LOG.log(Level.SEVERE, "IOException using JarThemeFactory's ClassLoader", ioex);
            }
        }
        return manifests;
    }

    private static void log(final String template, final Object... arguments) {
        LOG.finest(() -> String.format(template, arguments));
    }
    ...
    private Theme createTheme(Attributes themeAttributes, Locale locale,
        ...
        if (messageOverride != null) {
            try {
                ...
                new StringBuilder("The message resource file ");
